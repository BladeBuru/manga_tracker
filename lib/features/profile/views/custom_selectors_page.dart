...
class _CustomSelectorsPageState extends State<CustomSelectorsPage> {
  ...
  Future<void> _addSelector() async {
    ...
    SelectorType selectedType = SelectorType.urlPattern; // Par défaut : Pattern d'URL
          ...
          return AlertDialog(
            title: Row(
              children: [
                ...
                Expanded(
                  child: Text(l10n?.addCustomSelector ?? 'Ajouter un sélecteur'),
                ),
              ],
            ),
                ...
                crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    if (selectedType != SelectorType.urlPattern)
                      TextField(
                        ...
                        decoration: InputDecoration(
                          labelText: l10n?.selectorDomainLabel ?? 'Domaine (ex: exemple.com)',
                    ...
                    TextField(
                      ...
                      decoration: InputDecoration(
                        labelText: selectedType == SelectorType.urlPattern
                            ? (l10n?.selectorUrlPatternLabel ?? 'Pattern d\'URL (regex)')
                            : (l10n?.selectorCssLabel ?? 'Sélecteur CSS'),
                        hintText: selectedType == SelectorType.urlPattern
                            ? '/chapter-(\\d+)/'
                            : '.ad-banner, #ad-container',
                        border: const OutlineInputBorder(),
                        prefixIcon: const Icon(Icons.code),
                        helperText: selectedType == SelectorType.urlPattern
                            ? (l10n?.selectorUrlPatternHint ?? 'Exemple : /chapter-(\\d+)/ pour détecter /chapter-22')
                            : null,
                      ),
                      maxLines: selectedType == SelectorType.urlPattern ? 2 : 3,
                    ),
                  const SizedBox(height: 16),
                  DropdownButtonFormField<SelectorType>(
                    value: selectedType,
                    decoration: InputDecoration(
                      labelText: l10n?.selectorTypeLabel ?? 'Type de sélecteur',
                      border: const OutlineInputBorder(),
                    ...
                    items: [
                      DropdownMenuItem(
                        ...
                        child: Text(l10n?.selectorTypeUrlPattern ?? 'Pattern d\'URL'),
                      ),
                      DropdownMenuItem(
                        value: SelectorType.adBlocker,
                        child: Text(l10n?.selectorTypeAdBlocker ?? 'Bloqueur de publicités'),
                      ),
                      DropdownMenuItem(
                        value: SelectorType.chapterContent,
                        child: Text(l10n?.selectorTypeChapterContent ?? 'Contenu du chapitre'),
                      ),
                    ],
                    onChanged: (value) {
                      if (value != null) {
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      labelText: l10n?.selectorDescriptionLabel ?? 'Description (optionnel)',
                      hintText: l10n?.selectorDescriptionHint ?? 'Description du sélecteur',
                      border: const OutlineInputBorder(),
                        ...
                        child: Column(
                          ...
                          children: [
                            if (selectedType == SelectorType.adBlocker) ...[
                              Text(
                                l10n?.selectorExamplesAdBlocker ?? 'Exemples pour bloquer des publicités :',
                                style: const TextStyle(fontWeight: FontWeight.bold),
                              ),
                              const SizedBox(height: 8),
                              _buildExampleItem(
                                context,
                                l10n?.selectorExampleAd1 ?? 'Bannière publicitaire',
                                '.ad-banner, .ad-container',
                                'Bloque les éléments avec les classes ad-banner ou ad-container',
                              ),
                              _buildExampleItem(
                                context,
                                l10n?.selectorExampleAd2 ?? 'Publicité par ID',
                                '#advertisement, #ad-wrapper',
                                'Bloque les éléments avec les IDs advertisement ou ad-wrapper',
                              ),
                              _buildExampleItem(
                                context,
                                l10n?.selectorExampleAd3 ?? 'Iframe publicitaire',
                                'iframe[src*="ads"], iframe[src*="doubleclick"]',
                                'Bloque les iframes contenant "ads" ou "doubleclick" dans leur src',
                              ),
                              _buildExampleItem(
                                context,
                                l10n?.selectorExampleAd4 ?? 'Script publicitaire',
                                'script[src*="advertising"], script[id*="ad"]',
                                'Bloque les scripts de publicité',
                              ),
                            ] else if (selectedType == SelectorType.urlPattern) ...[
                              Text(
                                l10n?.selectorExamplesUrlPattern ?? 'Exemples de patterns d\'URL :',
                                style: const TextStyle(fontWeight: FontWeight.bold),
                              ),
                              const SizedBox(height: 8),
                                  ...
                                  children: [
                                    Row(
                                      children: [
                                        ...
                                        const SizedBox(width: 8),
                                        Text(
                                          l10n?.selectorExampleUrlPattern ?? 'Exemple : /chapter-22',
                                    ...
                                    _buildFieldExample(
                                      context,
                                      l10n?.selectorUrlPatternLabel ?? 'Pattern d\'URL',
                                      '/chapter-(\\d+)/',
                                  ...
                                  children: [
                                    Row(
                                      children: [
                                        ...
                                        const SizedBox(width: 8),
                                        Text(
                                          l10n?.selectorExampleChapter5 ?? 'Format manga/chapitre-22',
                                          style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: Colors.green),
                                        ),
                                      ],
                                    ),
                                    ...
                                    const SizedBox(height: 12),
                                    Text(
                                      l10n?.selectorWhenNeeded ?? 'Quand ajouter un sélecteur CSS ?',
                                      style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 12),
                                    ),
                                    const SizedBox(height: 8),
                                        ...
                                        children: [
                                          Row(
                                            children: [
                                              ...
                                              Text(
                                                l10n?.selectorPracticalExample ?? 'Exemple pratique :',
                                          ...
                                          _buildStepExample(
                                            context,
                                            '1',
                                            l10n?.selectorStep1 ?? 'Ouvrez la page du chapitre dans votre navigateur',
                                            'https://monsite.com/manga/chapppter-22',
                                          ),
                                          _buildStepExample(
                                            context,
                                            '2',
                                            l10n?.selectorStep2 ?? 'Appuyez sur F12 pour ouvrir les outils de développement',
                                            '',
                                          ),
                                          _buildStepExample(
                                            context,
                                            '3',
                                            l10n?.selectorStep3 ?? 'Cliquez sur l\'icône "Inspecter" (ou Ctrl+Shift+C)',
                                            '',
                                          ),
                                          _buildStepExample(
                                            context,
                                            '4',
                                            l10n?.selectorStep4 ?? 'Cliquez sur le conteneur qui contient les images du chapitre',
                                            '',
                                          ),
                                          _buildStepExample(
                                            context,
                                            '5',
                                            l10n?.selectorStep5 ?? 'Dans le code HTML, trouvez la classe ou l\'ID du conteneur',
                                          ...
                                          Container(
                                            ...
                                            child: Column(
                                              ...
                                              children: [
                                                Text(
                                                  l10n?.selectorFillForm ?? 'Remplissez le formulaire :',
                                                  style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 11),
                                                ),
                                                const SizedBox(height: 6),
                                                _buildFormFieldExample(
                                                  context,
                                                  l10n?.selectorDomainLabel ?? 'Domaine',
                                                  'monsite.com',
                                                ),
                                                const SizedBox(height: 6),
                                                _buildFormFieldExample(
                              ...
                              const SizedBox(height: 12),
                              Text(
                                l10n?.selectorOtherExamples ?? 'Autres exemples courants :',
                                style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 12),
                              ),
                              const SizedBox(height: 8),
                              _buildExampleItem(
                                context,
                                l10n?.selectorExampleChapter1 ?? 'Conteneur de chapitre',
                                '.chapter-content, .chapter-images',
                                'Identifie le conteneur principal du chapitre',
                              ),
                              _buildExampleItem(
                                context,
                                l10n?.selectorExampleChapter2 ?? 'Lecteur de manga',
                                '.manga-reader, .reader-content',
                                'Identifie la zone de lecture',
                              ),
                              _buildExampleItem(
                                context,
                                l10n?.selectorExampleChapter3 ?? 'Images du chapitre',
                                '.chapter-content img, .reading-content img',
                                'Identifie les images du chapitre',
                              ),
                              _buildExampleItem(
                                context,
                                l10n?.selectorExampleChapter4 ?? 'Contenu de lecture',
                ...
                onPressed: () {
                  if (selectedType == SelectorType.urlPattern) {
                    ...
                    if (selectorController.text.trim().isEmpty) {
                      _notifier.error(l10n?.selectorRequiredFields ?? 'Le pattern est requis');
                      return;
                    }
                  } else {
                    ...
                    if (domainController.text.trim().isEmpty ||
                      ...
                      _notifier.error(l10n?.selectorRequiredFields ?? 'Tous les champs sont requis');
                      return;
                    }
                  }
    ...
    if (result == true) {
      final selector = CustomSelector(
        ...
        domain: selectedType == SelectorType.urlPattern 
  ...
  Future<void> _deleteSelector(CustomSelector selector) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(l10n?.deleteSelectorConfirm ?? 'Êtes-vous sûr de vouloir supprimer ce sélecteur ?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
            child: Text(l10n?.cancel ?? 'Annuler'),
  ...
  Future<void> _exportSelectors() async {
    ...
    _notifier.success(l10n?.selectorsExported ?? 'Sélecteurs exportés dans le presse-papiers');
  }
  ...
  Future<void> _importSelectors() async {
    ...
    if (result == true) {
      ...
      final l10n = AppLocalizations.of(context);
      _notifier.success(
        l10n?.selectorsImported(count.toString()) ?? '$count sélecteur(s) importé(s)',
      );
    }
  }
  ...
  Future<void> _shareSelectors() async {
    ...
    if (await canLaunchUrl(uri)) {
      ...
      _notifier.info(l10n?.selectorsReadyToShare ?? 
              ...
              ? Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 16),
                      Text(
                        l10n?.noCustomSelectors ?? 'Aucun sélecteur personnalisé',
                        style: const TextStyle(fontSize: 18, color: Colors.grey),
                      ),
                      const SizedBox(height: 8),
                      Text(
                        l10n?.addFirstSelector ?? 'Ajoutez votre premier sélecteur pour commencer',
                        ...
                        leading: Icon(
                          selector.type == SelectorType.urlPattern
                              ...
                              : selector.type == SelectorType.adBlocker
                                  ? Icons.block
                                  : Icons.book,
                          color: selector.type == SelectorType.urlPattern
                              ? Colors.green
                              : selector.type == SelectorType.adBlocker
                                  ? Colors.red
                                  : Colors.blue,
                        ),
                        title: Text(selector.type == SelectorType.urlPattern 
                            ? (l10n?.selectorUrlPatternGlobal ?? 'Pattern global')
                            : selector.domain),
                        subtitle: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            ...
                            Chip(
                              label: Text(
                                selector.type == SelectorType.urlPattern
                                    ? l10n?.selectorTypeUrlPattern ?? 'Pattern URL'
                                    : selector.type == SelectorType.adBlocker
                                        ? l10n?.selectorTypeAdBlocker ?? 'Bloqueur de pub'
                                        : l10n?.selectorTypeChapterContent ?? 'Contenu',
  ...
  Widget _buildStepExample(BuildContext context, String stepNumber, String instruction, String example) {
    return Padding(
  ...
  Widget _buildFormFieldExample(BuildContext context, String label, String value, {bool isCode = false}) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        SizedBox(
        ...
        Expanded(
          child: Container(
            ...
            decoration: BoxDecoration(
              color: isCode ? Colors.black87 : Theme.of(context).colorScheme.surfaceVariant,
              borderRadius: BorderRadius.circular(4),
              border: Border.all(
            ...
            child: Text(
              ...
              style: TextStyle(
                ...
                fontSize: 10,
                color: isCode ? Colors.lightGreenAccent : Theme.of(context).colorScheme.onSurface,
              ),
            ),
          ),
        ),
  ...
  Widget _buildFieldExample(BuildContext context, String fieldName, String exampleValue, String explanation) {
    return Container(
      padding: const EdgeInsets.all(10),
      decoration: BoxDecoration(
  ...
  Widget _buildExampleItem(BuildContext context, String title, String selector, String description) {
    return Padding(
