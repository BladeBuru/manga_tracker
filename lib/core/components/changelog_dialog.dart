...
class _ChangelogDialogState extends State<ChangelogDialog> {
  ...
  Future<void> _translateChangelogs(
    ...
    try {
      ...
      final translatedVersions =
          changelogInfo.newVersions.map((vc) {
            return VersionChanges(
              ...
              notes:
                  vc.notes.map((note) {
                    ...
                    if (note is List)
                      return note.map((e) => e.toString()).join('\n');
      ...
      for (int v = 0; v < changelogInfo.newVersions.length; v++) {
        ...
        for (final note in versionChanges.notes) {
          ...
          if (noteText.isNotEmpty) {
            final cached = await _translationService
    ...
    return AlertDialog(
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children:
              changelogToDisplay.newVersions.map((changes) {
                return Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      ...changes.notes.map(
                        (note) => MarkdownBody(
                          data:
                              RegExp(r'^[#\-\*]').hasMatch("$note")
                                  ? "$note"
                                  : "- $note",
                          styleSheet: MarkdownStyleSheet(
                            p: TextStyle(fontSize: 14, color: onSurface),
                          ),
                        ),
                      ),
                    ],
